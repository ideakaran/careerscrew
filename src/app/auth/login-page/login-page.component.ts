...
import {APP_PROPERTIES, APP_ROUTES, QueryParamKey, QueryParamUIKey} from '@app/core/core.constant';
...
export class LoginPageComponent implements OnInit, OnDestroy {
  ...
  private processRouteQueryParams(): void {
    this.route.queryParams
      ...
      .subscribe((params: Params) => {
        ...
        this.originalRequestedUri = paramMap.get(QueryParamUIKey.ORIGINAL_REQUEST_URI);
        this.populateParamResponseMessage(paramMap, QueryParamKey.ERROR);
        this.populateParamResponseMessage(paramMap, QueryParamUIKey.DEFAULT_INFO_MESSAGE);
        this.populateParamResponseMessage(paramMap, QueryParamUIKey.PASSWORD_RESET_SUCCESSFUL);
        this.populateParamResponseMessage(paramMap, QueryParamUIKey.LOGOUT_SUCCESSFUL);
      });
  }

  populateParamResponseMessage(paramMap: ParamMap, paramKey: string, isErrorType: boolean = false): void {
  ...
  onLoginSubmit(): void {
    ...
    this.loading = true;
    const defaultRedirectUri = this.originalRequestedUri && this.originalRequestedUri === APP_ROUTES.LOGIN ?
      APP_ROUTES.HOME : this.originalRequestedUri;
    this.authenticationService.login(loginContext, true, defaultRedirectUri)
      .pipe()
      .subscribe(
        res => {
          this.loading = false;
          this.hasError = false;
          this.loginResponseMessage = 'Login Successful. Please wait...';
  ...
  processForgotPasswordReset() {
    this.loading = true;
    this.authenticationService.sendEmailForforgotPassword(this.emailAddressToResetPassword)
      .pipe()
      .subscribe(
        res => {
          this.loading = false;
